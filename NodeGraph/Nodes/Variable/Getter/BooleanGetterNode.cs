...
public class BooleanGetterNode : Node, IGetter
{
    public BooleanGetterNode(ScriptBrain brain, VariableData data, int destinationId = -1, string destinationPin = "") : base(brain, NodeTypes.Boolean_Getter)
    {
        ...
        if (destinationId != -1)
        {
            Input.Add(new VariableConnection(typeof(bool), typeof(BooleanGetterNode), destinationId, destinationPin, NodeId, Keywords.Value));
        }
    }

    public VariableConnection Getter(ScriptBrain brain, int destinationId = -1, string destinationPin = "")
    {
        return new VariableConnection(typeof(bool), typeof(BooleanGetterNode), destinationId, destinationPin, NodeId,
            Keywords.Value);
    }
}