...
{
    public MultiplyNumberNode(ScriptBrain brain, IGetter a, IGetter b) : base(brain, NodeTypes.MultiplyNumber)
    {
    ...
    public VariableConnection Getter(ScriptBrain brain, int destinationId = -1, string destinationPin = "")
    {
        if (Input.Count != 2 || Input[0] is not VariableConnection a || Input[1] is not VariableConnection b)
            throw new InvalidDataException($"Trying to get result from a MultiplyNumberNode with invalid inputs");
